...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            User::create([
                ...
                'Email' => $request->email,
                'Phone' => $request->phone,
                'password' => $request->password, // Sẽ được hash bởi setPasswordAttribute
                'UserType' => 'Regular',
            ]);
        } catch (\Exception $e) {
    ...
    public function login(Request $request)
    {
        ...
        if ($request->login_input === 'admin' && $request->password === 'admin') {
            session([
                'currentUser' => [
                    'name' => 'Admin',
                    'email' => 'admin',
        ...
        if (Auth::attempt($credentials, $request->has('checkbox'))) {
            ...
            $user = Auth::user();
            // Lưu thông tin user vào session (để tương thích với code cũ)
            session([
                'currentUser' => [
                    'name' => $user->FullName,
                    'email' => $user->Email,
